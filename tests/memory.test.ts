...
describe('KeyvDataLoader with memory store', () => {
  ...
  describe('basic functionality', () => {
    ...
    test('should batch multiple loads', async () => {
      const promises = [loader.load('key2'), loader.load('key3')];
      const results = await Promise.all(promises);

      expect(results).toEqual(['Value for key2', 'Value for key3']);
    ...
    test('should load many keys', async () => {
      ...
      expect(results).toEqual(['Value for key4', 'Value for key5']);
